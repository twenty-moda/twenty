...
export type SubmitProofResult = { ok: true; change: StatusChange | null } | { ok: false; message: string };
...
export async function submitPaymentProof(db: Db, orderId: string, proof: { image: Buffer; width: number; height: number }): Promise<SubmitProofResult> {
  ...
  await db.insert(paymentProofs).values({ orderId, image: proof.image, contentType: "image/webp", width: proof.width, height: proof.height });
  if (order.status !== "pendiente") return { ok: true, change: null };
  ...
  return { ok: true, change: result.ok ? result.change : null };
}
