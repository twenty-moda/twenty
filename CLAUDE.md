...
- **Decisión:** en lugar de Auth.js (v5 nunca salió de beta y el proyecto pasó a Better Auth) se usa un módulo propio pequeño. Sesiones en la tabla `sessions` con `scope`: `admin` (cookie `twenty_session`, email + contraseña) o `cuenta` (cookie `twenty_cuenta`, Google). Una sesión de la cuenta nunca vale para el panel, aunque el email sea de un admin.
- **Cuentas de cliente (pedido de TWENTY, 24/09/2026):** se entra solo con Google usando **Firebase Authentication**, pero Firebase solo confirma quién es:
  - `/ingresar` (`components/account/google-sign-in.tsx`) abre la ventana de Google con el SDK de Firebase (cargado solo en esa página, persistencia en memoria) y manda el ID token a `signInWithGoogleAction`.
  - El servidor lo verifica con las llaves públicas de Google (`server/services/firebase-auth.ts`, con `jose`, sin cuenta de servicio): proyecto, vencimiento, email verificado, proveedor `google.com` e ingreso de hace menos de 10 min.
  - `accounts.ts` busca la cuenta por `users.firebase_uid` o por email (vincula cuentas que ya existían sin cambiarles el rol) y se abre la sesión propia.
  ...
  - Comprar sin cuenta sigue igual. Google no deja entrar desde el navegador de Instagram, Facebook o TikTok: `/ingresar` lo detecta y pide abrir la página en Chrome o Safari.
- **Firebase:** proyecto `twenty-moda` (cuenta de Google de TWENTY), app web "TWENTY tienda web". Config en `NEXT_PUBLIC_FIREBASE_*` (pública por diseño; copia local en `.env.firebase`, ignorado por git; en Vercel solo Production).
  - Google se activó con `npx firebase-tools deploy --only auth --project twenty-moda` usando un `firebase.json` con `auth.providers.googleSignIn` (nombre "TWENTY" y email de soporte de TWENTY). Ese archivo no está en el repo.
...
- **Emails:** `server/services/email.ts` (Resend por HTTP, `RESEND_API_KEY` + `EMAIL_FROM`, reintento en 429/5xx, `Idempotency-Key`). Diseño en `email-layout.ts`: el email es una lista de bloques que se convierte a HTML con tablas (franja negra con `public/brand/twenty-logo-email.png`) y a texto plano. Se envían con `after()` para no demorar la respuesta. En los logs nunca va el asunto ni el destinatario (datos personales): solo el `tag`.
  ...
  - **Remitente:** `TWENTY <pedidos@twentymoda.com>` en Vercel (Production). Hasta verificar twentymoda.com en Resend (registros DNS en HostGator) todo envío falla con 403 y queda en el log. La llave es "Sending access": no gestiona dominios. Plan gratis: 100 emails/día y 3.000/mes.
